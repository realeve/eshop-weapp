...
export const getAlipayPrepayId: <T>(
  ...
  let url = payment === PAYMENT.mp ? API.BUY_STEP4_MP : false;
  if (!url) {
    fail("不支持的支付方式");
...
// 因微信小程序支付生态的限制，直接调用微信支付
export const getMpPrepayId: <T>(data: IPayReq) => Promise<T> = data =>
  getAlipayPrepayId(PAYMENT.mp, data);
