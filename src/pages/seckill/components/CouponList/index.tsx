...
const initData = [
  {
    ...
    hour: '10:00',
    title: '悦之品 海鲜礼盒提货券 礼品卡送礼 海鲜礼券 电子券 电子券电子券电子券',
    tip: '礼品企业团购员工福利',
    total: '200',
    rest: '0',
    price: '1245',
...
const CouponList: (prop: IPropData) => React.ReactElement = ({ id, list }) => {
  ...
  const { data } = useFetch({
    param: { ...(SECKILL.detail as {}), data: { page: 1, scheduleId: current } },
  ...
  return (
    <View>
      {list && <BuyTime limitTime={list} onClick={setCurrent} current={current} />}
      <div className='popular'>
        {data && data.dist ? (
          (data.dist || []).map(item => (
            <CouponItem state={item.state} current={current} data={item} key={item.id} />
          ))
        ) : (
          <CEmpty type="goods" />
        )}
